...
const NotificationsPanel = () => {
  return (
    <div className="bg-white rounded-[40px] p-8 border border-gray-100 shadow-sm flex flex-col h-[450px]">
      <div className="flex items-center justify-between mb-8 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-xl font-black text-brand-deep italic">შეტყობინებები</h3>
        </div>
      ...
      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-4">
        {notifications.map((note) => (
          <div key={note.id} className="p-5 rounded-[28px] border border-gray-50 hover:border-brand-purple/20 hover:bg-slate-50 transition-all group cursor-pointer">
            <div className="flex gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start mb-1">
                  <h4 className="font-black text-brand-deep text-sm truncate">{note.title}</h4>
      ...
      <button className="w-full mt-6 py-4 rounded-2xl border-2 border-dashed border-gray-100 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 hover:border-brand-purple hover:text-brand-purple transition-all shrink-0">
        ყველას ნახვა
      </button>
    </div>
  );
