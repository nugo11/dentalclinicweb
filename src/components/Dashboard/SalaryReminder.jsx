...
const SalaryReminder = ({ staff = [], paidStaffIds = [] }) => {
  ...
  return (
    <>
      <div className="bg-white p-6 rounded-[32px] border border-slate-200/60 shadow-sm overflow-hidden relative group">
        <div className="flex justify-between items-center mb-6 relative z-10">
          <div>
            <h3 className="font-black text-brand-deep uppercase tracking-tighter text-[11px] italic">ხელფასების შეხსენება</h3>
            <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">მიმდინარე პერიოდი</p>
          </div>
          ...
          {displaySalaries.map((alert) => (
            <div 
              ...
              className={`p-4 rounded-2xl border transition-all flex items-center justify-between
                ${alert.isPaid ? 'bg-emerald-50 border-emerald-100' : 
                  alert.isToday ? 'bg-amber-50 border-amber-100 animate-pulse' : 'bg-slate-50 border-transparent'}`}
            >
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center
                  ${alert.isPaid ? 'bg-emerald-500 text-white' : 
                    alert.isToday ? 'bg-amber-500 text-white' : 'bg-slate-200 text-slate-500'}`}>
                  {alert.isPaid ? <CheckCircle2 size={14} /> : alert.isToday ? <AlertCircle size={14} /> : <Wallet size={14} />}
                </div>
                <div>
                  <p className="text-[10px] font-black text-brand-deep uppercase">{alert.fullName}</p>
                  <p className={`text-[9px] font-bold uppercase ${alert.isPaid ? 'text-emerald-600' : 'text-slate-500'}`}>
              ...
              {!alert.isPaid && (
                <button 
                  ...
                  className={`p-2 rounded-xl transition-all
                    ${alert.isToday ? 'bg-amber-500 text-white hover:bg-amber-600' : 'bg-white text-slate-400 hover:text-brand-purple'}`}
                >
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-[40px] w-full max-w-sm p-10 relative z-10 shadow-2xl animate-in zoom-in-95 duration-200">
             <div className="text-center mb-8">
                ...
                <h3 className="text-2xl font-black text-brand-deep italic tracking-tighter">ხელფასის გაცემა</h3>
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-2">{paymentModal.member?.fullName}</p>
             </div>
             ...
             <div className="space-y-6">
                <div className="space-y-3">
                   ...
                   <div className="space-y-1">
                      <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest ml-4">გადასახდელი თანხა (₾)</label>
                      <input 
                         type="number" 
                         autoFocus
                         className="w-full bg-slate-50 border-2 border-transparent focus:border-brand-purple rounded-2xl px-6 py-5 outline-none font-black text-xl text-brand-deep transition-all placeholder:text-slate-200"
