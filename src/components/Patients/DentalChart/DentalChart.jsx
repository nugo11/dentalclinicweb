...
const DentalChart = ({ data, onToothUpdate }) => {
  ...
  return (
    <div className="bg-surface rounded-[40px] p-4 sm:p-10 border border-border-main shadow-sm overflow-hidden w-full">
      {/* ლეგენდა */}
      <div className="flex flex-wrap gap-6 mb-12 pb-8 border-b border-border-main justify-center">
      ...
      <div className="space-y-16">
        ...
        <div className="flex flex-col items-center">
          <div className="px-6 py-2 bg-surface-soft rounded-full mb-8 border border-border-main">
            <span className="text-[10px] font-black text-text-muted uppercase tracking-[0.3em]">ზედა ყბა</span>
          </div>
          <div className="flex flex-wrap justify-center gap-6 max-w-full">
            ...
            {/* გამყოფი ხაზი */}
            <div className="hidden lg:block w-px h-24 bg-surface-soft mx-2" /> 
            {/* ზედა მარჯვენა */}
            <div className="flex gap-2 flex-wrap justify-center">
        ...
        <div className="flex flex-col items-center">
          <div className="px-6 py-2 bg-surface-soft rounded-full mb-8 border border-border-main">
            <span className="text-[10px] font-black text-text-muted uppercase tracking-[0.3em]">ქვედა ყბა</span>
          </div>
          <div className="flex flex-wrap justify-center gap-6 max-w-full">
            {/* ქვედა მარცხენა */}
            <div className="flex gap-2 flex-wrap justify-center">
...
const LegendItem = ({ color, label }) => (
  <div className="flex items-center gap-3 bg-surface-soft/50 px-4 py-2.5 rounded-2xl border border-transparent hover:border-border-main transition-all">
    <div className={`w-3 h-3 rounded-full ${color} shadow-sm`} />
    <span className="text-[10px] font-black uppercase text-text-main tracking-widest">{label}</span>
  </div>
);
